...
All inputs in group chat mode are treated as <command> + <arg1>, where the first word encountered in input is <command>, and all other words following is treated as single argument <arg1>. Note that <arg1> may be empty string, if no inputs are found.
...
This is because of the blocking nature of I/O in C, difficulty encountered during attempting to read stdin, and print editable outputs (which alone seems to be an equally big project as the entire PA1). I choose this way to display the message as it is implementation-wise the easiest.
